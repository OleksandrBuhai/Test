...
const Line: React.FC<{ x1: number; y1: number; x2: number; y2: number }> = ({
  x1,
  y1,
  x2,
  y2,
...
const DynamicDiv: React.FC<DynamicDivProps> = ({}) => {
  ...
  const handleAddChild = () => {
    ...
    const newChild = (
      <DynamicDiv key={childCount} id={childCount + 1} name={newChildName} />
    );
  ...
  return (
    <div
      style={{
        ...
        padding: "10px",
        margin: "10px",
        textAlign: children.length === 2 ? "center" : "left",
      }}
    >
      <div
        style={{
  ...
  const handleAddChild = () => {
    const newChild = (
      <DynamicDiv
        ...
        name={`Child Div ${children.length + 1}`}
      />
    );
    setChildren((prevChildren) => [...prevChildren, newChild]);
  };
